...
<script>
	...
	function fCheck() {
		...
		if(fileSize > maxSize) alert("업로드할 파일의 최대 용량은 20MB입니다");
		else if(ext != "jpg" && ext != "jpeg" && ext != "gif" && ext != "png" && ext != "zip" && ext != "ppt" && ext != "pptx" && ext != "hwp" && ext != "txt") {
			alert("업로드 가능한 파일은 jpg/jpeg/gif/png/zip/ppt/pptx/hwp/txt 파일입니다");
		}
		else {
			myform.submit();
		}
...
<div class="container">
	...
	<form name="myform" method="post" enctype="multipart/form-data">
		...
		<p>파일명: 
			<input type="file" name="fName" id="fName" class="form-control-file border" accept=".jpg,.gif,.png,.jpeg,.zip,.pptx,.ppt,.hwp,.txt" />
		</p>
		<p>
	...
	<div id="downloadFile">
		...
		<div class="row">
			...
			<div class="col text-right">
				<input type="button" value="폴더내 모든파일 삭제" onclick="fileDeleteAll()" class="btn btn-danger btn-sm mb-2"/>
			</div>
		</div>
			...
			<c:forEach var="file" items="${files}" varStatus="st">
				<tr>
					...
					<td>
						...
						<c:if test="${extName == 'jpg' || extName == 'jpeg' || extName == 'png' || extName == 'gif'}">
